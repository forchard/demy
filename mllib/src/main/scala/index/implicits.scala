...
object implicits {
  implicit class DatasetUtil(val left: Dataset[_]) {
    def luceneLookup(right:Dataset[_], query:Column, text:Column, maxLevDistance:Int=0, indexPath:String, reuseExistingIndex:Boolean=false
                   ...
                   , tokenizeText:Boolean=true, minScore:Double=0.0, boostAcronyms:Boolean=false, termWeightsColumnName:Option[String]=None
      ...
      val indexFiles =  indexNode.list.toArray
                          .map(node => SparkLuceneReader(indexPartition=node.path,  reuseSnapShot = true
                                                      ...
                                                      , indexStrategy = strategy, strategyParams = strategyParams))
               ...
               rowsChunk = rowsChunk.map(elem => elem match{ case (leftRow, righResults) =>
                 (leftRow, {
                       ...
                       val termWeightsArray:Seq[Option[Seq[Double]]] = {
                          if(isArrayJoinWeights.isEmpty) queries.map(_ => None)
                          else if(isArrayJoinWeights.get) Seq(Some(leftRow.getAs[Seq[Double]](leftRow.fieldIndex(termWeightsColumnName.get))))
                          else leftRow.getAs[Seq[Seq[Double]]](leftRow.fieldIndex(termWeightsColumnName.get)).map(value => Some(value))
//                          else if(isArrayJoinWeights.get) leftRow.getSeq[Seq[Double]](leftRow.fieldIndex(termWeightsColumnName.get)).map(value => Some(value.toArray))
//                          else leftRow.getAs[Seq[Seq[Double]]](leftRow.fieldIndex(termWeightsColumnName.get)).map(value => Some(value.toArray))
                       }
                       ...
                       Some(
                           queries.zipWithIndex.map(p => p match {case (query, i) => {
                               ...
                               val res:Array[GenericRowWithSchema] = rInfo.search(query=query, maxHits=1, filter = Row.empty, outFields=rightRequestFields,
