...
case class ModelVersion(steps:Seq[ModelStep]=Seq[ModelStep](), comment:String="", customLogs:GenericRowWithSchema=new GenericRowWithSchema(values= Array[Any](), schema=new StructType(Array[StructField]()))) {
    ...
    def step(stepChoices:(String, String)*) = {
        ...
        if(noMatchs.size > 0) warning(s"Step(s) ${noMatchs.mkString(", ")} should exists exactly once among (${this.steps.map(step =>  (step.name, step.version)).mkString})")
        ...
        val newSteps = this.steps
                            ...
                            .map(step => if(choices.filter(c => c.step == step.name && c.version == step.version ).size == 1) step.option("log"->"true") else step)
    ...
    def drop(stepsToDrop:String*) = {
        ...
        if(nonExisting.size > 0) warning(s"Cannot fin the following(s) step to drop: ${nonExisting.mkString(",")} between ${allSteps.mkString(",")}")
    ...
    def dropAfter(lastStep:String) = {
        var afterLast = false
        val newSteps = this.steps.iterator.takeWhile((step)=>{val isAfter = afterLast; if(step.name==lastStep) {afterLast = true}; !isAfter}).toArray.toSeq
    ...
    def params(step:String, paramPairs:(String, Any)*):ModelVersion = {
        ...
        if(versionNames.size == 0) warning(s"Cannot find step ${step}: between [${this.steps.map(step => step.name).mkString(", ")}]")
    ...
    def params(step:String, version:String, paramPairs:(String, Any)*):ModelVersion = {
        ...
        val theSteps = this.steps.map(s => if(s.name == step && s.version == version) s.applyParams(stepParams:_*).option("log"->"true") else s)
